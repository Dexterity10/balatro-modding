LOVELY_INTEGRITY = '142bcaf541b2404de2d28d425e1ee8aa080020e7391857263dc27067373c49d4'

SMODS.DrawSteps = {}
SMODS.DrawStep = SMODS.GameObject:extend{
    obj_table = SMODS.DrawSteps,
    obj_buffer = {},
    required_params = {'key', 'order', 'func'},
    layers = {
        card = true,
        both = true
    },
    ...
    post_inject_class = function(self)
        table.sort(self.obj_buffer, function(_self, _other)
            return self.obj_table[_self].order < self.obj_table[_other].order
        end)
    end,
    conditions = {},
    check_individual_condition = function(self, card, layer, k, v)
        if k == 'vortex' then
            return not not card.vortex == v
        end
        if k == 'facing' then
            return card.sprite_facing == v
        end
        return true
    end,
    check_conditions = function(self, card, layer)
        ...
        for k, v in pairs(self.conditions) do
            if not self:check_individual_condition(card, layer, k, v) then
                return
            end
        end
        return true
...
SMODS.DrawStep {
    ...
    func = function(self)
        ...
        if not self.no_shadow and G.SETTINGS.GRAPHICS.shadows == 'On' and
            ((self.ability.effect ~= 'Glass Card' and not self.greyed and self:should_draw_shadow()) and
            ...
            self.shadow_height = 0 * (0.08 + 0.4 * math.sqrt(self.velocity.x ^ 2)) +
                                     ((((self.highlighted and self.area == G.play) or self.states.drag.is) and 0.35) or
                                         (self.area and self.area.config.type == 'title_2') and 0.04 or 0.1)
...
SMODS.DrawStep {
    ...
    func = function(self, layer)
        ...
        if not self.config.center.discovered and (self.ability.consumeable or self.config.center.unlocked) and
            ...
            if (self.config.center.undiscovered and not self.config.center.undiscovered.no_overlay) or
                not (SMODS.UndiscoveredSprites[self.ability.set] and
                    SMODS.UndiscoveredSprites[self.ability.set].no_overlay) then
                shared_sprite:draw_shader('dissolve', nil, nil, nil, self.children.center, scale_mod, rotate_mod)
            else
                if SMODS.UndiscoveredSprites[self.ability.set] and
                    SMODS.UndiscoveredSprites[self.ability.set].overlay_sprite then
                    SMODS.UndiscoveredSprites[self.ability.set].overlay_sprite:draw_shader('dissolve', nil, nil, nil,
...
SMODS.DrawStep {
    ...
    func = function(self, layer)
        ...
        if (self.edition and self.edition.negative) or
            (self.ability.name == 'Antimatter' and (self.config.center.discovered or self.bypass_discovery_center)) then
            if self.children.front and
                (self.ability.delayed or
                    (self.ability.effect ~= 'Stone Card' and not self.config.center.replace_base_card)) then
...
SMODS.DrawStep {
    ...
    func = function(self)
        if (self.ability.set == 'Voucher' or self.config.center.demo) and
            (self.ability.name ~= 'Antimatter' or not (self.config.center.discovered or self.bypass_discovery_center)) then
            if self:should_draw_base_shader() then
                self.children.center:draw_shader('voucher', nil, self.ARGS.send_to_shader)
            end
        end
            ...
            for k, v in pairs(G.P_CENTER_POOLS.Edition) do
                if self.edition[v.key:sub(3)] and v.shader then
                    if type(v.draw) == 'function' then
                        ...
                        if self.children.front and self.ability.effect ~= 'Stone Card' and
                            not self.config.center.replace_base_card then
...
SMODS.DrawStep {
    ...
    func = function(self, layer)
        ...
        elseif (self.sticker_run and G.shared_stickers[self.sticker_run]) and G.SETTINGS.run_stake_stickers then
            ...
            G.shared_stickers[self.sticker_run]:draw_shader('voucher', nil, self.ARGS.send_to_shader, nil,
                self.children.center)
        end
        ...
        for k, v in pairs(SMODS.Stickers) do
            if self.ability[v.key] then
                if v and v.draw and type(v.draw) == 'function' then
                    ...
                    G.shared_stickers[v.key]:draw_shader('voucher', nil, self.ARGS.send_to_shader, nil,
                        self.children.center)
                end
            end
        end
...
SMODS.DrawStep {
    ...
    func = function(self)
        if self.ability.name == 'The Soul' and (self.config.center.discovered or self.bypass_discovery_center) then
            ...
            local rotate_mod = 0.1 * math.sin(1.219 * G.TIMERS.REAL) + 0.07 * math.sin((G.TIMERS.REAL) * math.pi * 5) *
    ...
    func = function(self)
        if self.config.center.soul_pos and (self.config.center.discovered or self.bypass_discovery_center) then
            local scale_mod = 0.07 + 0.02 * math.sin(1.8 * G.TIMERS.REAL) + 0.00 *
                                  math.sin((G.TIMERS.REAL - math.floor(G.TIMERS.REAL)) * math.pi * 14) *
                                  (1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL))) ^ 3
            local rotate_mod = 0.05 * math.sin(1.219 * G.TIMERS.REAL) + 0.00 * math.sin((G.TIMERS.REAL) * math.pi * 5) *
                                   (1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL))) ^ 2
                ...
                for k, v in pairs(G.P_CENTER_POOLS.Edition) do
                    if v.apply_to_float then
                        if self.edition[v.key:sub(3)] then
                            self.children.floating_sprite:draw_shader(v.shader, nil, nil, nil, self.children.center,
...
SMODS.DrawStep {
    ...
    func = function(self)
        if self.sticker and G.shared_stickers[self.sticker] then
            ...
            G.shared_stickers[self.sticker]:draw_shader('dissolve', nil, nil, true, self.children.center, nil,
                self.sticker_rotation, sticker_offset.x, sticker_offset.y)
            local stake = G.P_STAKES['stake_' .. string.lower(self.sticker)] or {}
            if stake.shiny then
                G.shared_stickers[self.sticker]:draw_shader('voucher', nil, self.ARGS.send_to_shader, true,
                    self.children.center)
            end
        end
    end,
...
SMODS.DrawStep {
    ...
    func = function(self)
        for k, v in pairs(self.children) do
            if not v.custom_draw and not SMODS.draw_ignore_keys[k] then
                v:draw()
            end
        end
    end
